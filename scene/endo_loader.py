...
class CholecSeg8k_Dataset(object):
    ...
    def load_meta(self):
        ...
        self.depth_paths = sorted(glob.glob(os.path.join(self.root_dir, 'depth_mu', "*.png")))

        self.lang_feature_paths = sorted(glob.glob(os.path.join(self.root_dir, 'language_features_agg_dim3', "*.npy")))
        # load poses
        poses = np.eye(4)[None]
    ...
    def format_infos(self, split):
        ...
        for idx in tqdm(idxs):
            ...
            lang_features = torch.from_numpy(np.load(self.lang_feature_paths[idx]))
            lang_features = F.interpolate(lang_features.permute(0, 3, 1, 2), \
                size=(self.H, self.W))[0] # [1, 3, 192, 192] -> [3, h, w]
            # color
            color = np.array(Image.open(self.image_paths[idx]))/255.0
            ...
            cameras.append(Camera(colmap_id=idx, R=R, T=T, FoVx=FovX, FoVy=FovY,image=image, depth=depth, mask=mask, gt_alpha_mask=None,
                          ...
                          Znear=None, Zfar=None, K=self.K, h=self.H, w=self.W, lang_features=lang_features))
        return cameras
    ...
    def get_sparse_pts(self, sample=True):
        ...
        depth = cv2.imread(self.depth_paths[0], -1)
    ...
    def search_pts_colors_with_motion(self, ref_pts, ref_color, ref_mask, ref_c2w):
        ...
        for j in range(1,  len(self.image_poses), interval):
            ...
            depth = cv2.imread(self.depth_paths[j], -1)
...
class CaDisv2_Dataset(object):
    def __init__(
        ...
        # print("test_idxs:", self.test_idxs)
        self.video_idxs = [i for i in range(n_frames)]
    ...
    def load_meta(self):
        ...
        agg_fn = lambda filetype: sorted(glob.glob(os.path.join(self.root_dir, filetype, "*.png")))
        self.image_paths = agg_fn("images")
        # self.depth_paths = sorted(glob.glob(os.path.join(self.root_dir, 'depths', "*.npy")))
        self.depth_paths = sorted(glob.glob(os.path.join(self.root_dir, 'depth_mu', "*.png")))
