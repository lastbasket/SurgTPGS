...
from scene.lang_deform import LanguageDeformation
...
class GaussianModel:
    ...
    def __init__(self, sh_degree, args):
        ...
        if self.args.include_feature:
            self.lang_deformation=LanguageDeformation(args)
        self.setup_functions()

    def capture(self, include_feature=False):
        if include_feature:
            assert self._language_feature is not None, "no language feature"
            assert self._language_opa is not None, "no language opacity"
            return (
                ...
                self.denom,
                self.LanguageDeformation.state_dict(),
    ...
    def restore(self, model_args, training_args):
        if len(model_args) == 13: # 这是一个feature训练时保存的ckpt
            ...
            opt_dict, 
            self.spatial_lr_scale) = model_args
            if not self.args.include_feature: # 如果是以原始gs为初始化来训练feature的话，就不需要restore optimizer
    ...
    @property
    def get_language_opa(self):
        if self._language_opa is not None:
            return self._language_opa
        else:
            raise ValueError('没有设置language feature')
    ...
    def create_from_pcd(self, pcd, spatial_lr_scale, time_line):
        ...
        self.lang_deformation = self.lang_deformation.cuda()
    ...
    def training_setup(self, training_args):
        ...
        if self.args.include_feature:
            if self._language_feature is None or self._language_feature.shape[0] != self._xyz.shape[0]:
                ...
                language_opa = torch.zeros((self._xyz.shape[0], 1), device="cuda")
                self._language_opa = nn.Parameter(language_opa.requires_grad_(True))
            ...
            l = [
                ...
                {'params': [self._language_opa], 'lr': training_args.opacity_lr, "name": "language_opa"},
                {'params': list(self.lang_deformation.parameters()), 'lr': training_args.language_net_lr, "name": "language_net"},
            ]
            ...
            assert self._language_opa is None, "在训练原始gs的时候language feature应该始终为None"
    ...
    def load_model(self, path):
        ...
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
        weight_dict_lang = torch.load(os.path.join(path,"lang_deformation.pth"),map_location="cuda")
        self.lang_deformation.load_state_dict(weight_dict_lang)
        self.lang_deformation = self.lang_deformation.cuda()
    ...
    def save_ply(self, path):
        ...
        lang_feature = self._language_feature.detach().cpu().numpy()
        lang_opa = self._language_opa.detach().cpu().numpy()
        
        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes()]

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, coefs, lang_feature, lang_opa), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
        
    def save_lang_deform(self,path):
        torch.save(self.lang_deformation.state_dict(),os.path.join(path, "lang_deformation.pth"))
    ...
    def load_ply(self, path):
        ...
        lang_opa = np.asarray(plydata.elements[0]["opa_lang"])[..., np.newaxis]
        ...
        lang_fea_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("lang_")]
        lang_fea_names = sorted(lang_fea_names, key = lambda x: int(x.split('_')[-1]))
        lang_fea = np.zeros((xyz.shape[0], len(lang_fea_names)))
        for idx, attr_name in enumerate(lang_fea_names):
            lang_fea[:, idx] = np.asarray(plydata.elements[0][attr_name])
        ...
        self._language_feature = nn.Parameter(torch.tensor(lang_fea, dtype=torch.float, device="cuda").requires_grad_(True))
        self._language_opa = nn.Parameter(torch.tensor(lang_opa, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def prune_points(self, mask):
        ...
        self._language_opa = optimizable_tensors["language_opa"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, 
                              new_language_feature, new_language_opa, new_opacities, 
                              new_scaling, new_rotation, new_coefs, new_deformation_table):
        d = {"xyz": new_xyz,
        "f_dc": new_features_dc,
        "f_rest": new_features_rest,
        "opacity": new_opacities,
        "language_feature": new_language_feature,
        "language_opa": new_language_opa,
        "scaling" : new_scaling,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_language_opa = self._language_opa[selected_pts_mask].repeat(N,1)
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, 
                                   new_language_feature, new_language_opa, new_opacity, 
                                   new_scaling, new_rotation, new_coefs,new_deformation_table)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_language_opa = self._language_opa[selected_pts_mask]
    ...
    def gaussian_deformation(self, t, ch_num = 10, basis_num = 17):
        ...
        # e^((t-mu)^2/(sigma^2))*weight
        weight, mu, sigma = torch.chunk(coefs,chunks=3,dim=-2)     # (N, ch_num, 1, basis_num)                  
        exponent = (t - mu)**2/(sigma**2+1e-4)      # (N, ch_num, 1, basis_num)
        gaussian =  torch.exp(-exponent**2)     # (N, ch_num, 1, basis_num)
        return (gaussian*weight).sum(-1).squeeze()      # (N, ch_num)
    
    def deformation(self, xyz, scales, rotations, time, lang_fea=None):
        """
        ...
        # print(deform.min().item(), deform.max().item())

        deform_xyz = deform[:, :3]      # [N, 3]
        xyz += deform_xyz
        deform_rot = deform[:, 3:7]     # [N, 3]
        rotations += deform_rot
              # [N, 3]
        try:
            # when ch_num is 10
            deform_scaling = deform[:, 7:10]        # [N, 3]
            scales += deform_scaling
            
            if lang_fea is not None:
                deform_language = deform[:, 10:13]
                # ablation
                # lang_fea += deform_language
                
                # main
                lang_fea = self.lang_deformation(lang_fea, deform_language,time)
                
                # for ablation 
                # lang_fea = lang_fea
                return xyz, scales, rotations, lang_fea
            else: 
                return xyz, scales, rotations
        except:
            if lang_fea is not None:
                deform_language = deform[:, 7:10]
                # ablation
                # lang_fea += deform_language
